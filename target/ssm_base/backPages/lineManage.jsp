<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
<%@ page contentType="text/html;charset=UTF-8" language="java" isELIgnored="false" %>
<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>城市公交后台管理</title>

  <!-- Google Font: Source Sans Pro -->
  <link rel="stylesheet"
    href="https://fonts.googleapis.com/css?family=Source+Sans+Pro:300,400,400i,700&display=fallback">
  <!-- Font Awesome -->
  <link rel="stylesheet" href="${pageContext.request.contextPath}/plugins/fontawesome-free/css/all.min.css">
  <!-- DataTables -->
  <link rel="stylesheet" href="${pageContext.request.contextPath}/plugins/datatables-bs4/css/dataTables.bootstrap4.min.css">
  <link rel="stylesheet" href="${pageContext.request.contextPath}/plugins/datatables-responsive/css/responsive.bootstrap4.min.css">
  <link rel="stylesheet" href="${pageContext.request.contextPath}/plugins/datatables-buttons/css/buttons.bootstrap4.min.css">
  <!-- Theme style -->
  <link rel="stylesheet" href="${pageContext.request.contextPath}/dist/css/adminlte.min.css">
  <!-- toast弹出框 -->
  <link rel="stylesheet" href="${pageContext.request.contextPath}/dist/css/toast.min.css">
</head>

<body class="hold-transition sidebar-mini">
  <div class="wrapper">
    <!-- Navbar -->
    <nav class="main-header navbar navbar-expand navbar-white navbar-light">
      <!-- Left navbar links -->
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link" data-widget="pushmenu" href="#" role="button"><i class="fas fa-bars"></i></a>
        </li>
        <li class="nav-item d-none d-sm-inline-block">
          <a href="#" class="nav-link">闲置链接</a>
        </li>
      </ul>

      <!-- Right navbar links -->
      <ul class="navbar-nav ml-auto">
        <li class="nav-item">
          <a class="nav-link" href="#" role="button">
            当前管理员：<span>${sessionScope.managerAccount}</span>
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="#" role="button">
            <button type="button" class="btn btn-primary btn-sm" data-toggle="modal"
              data-target="#modal_1">修改密码</button>
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="${pageContext.request.contextPath}/managerLogout" role="button">
            <button type="button" class="btn btn-primary btn-sm">退出登录</button>
          </a>
        </li>

      </ul>
    </nav>
    <!-- /.navbar -->

    <!-- Main Sidebar Container -->
    <aside class="main-sidebar sidebar-dark-primary elevation-4">
      <!-- Brand Logo -->
      <a href="${pageContext.request.contextPath}/findAllBusLine" class="brand-link">
        <img src="${pageContext.request.contextPath}/dist/img/AdminLTELogo.png" alt="AdminLTE Logo" class="brand-image img-circle elevation-3"
          style="opacity: .8">
        <span class="brand-text font-weight-light">公交后台管理</span>
      </a>

      <!-- Sidebar -->
      <div class="sidebar">

        <!-- SidebarSearch Form -->
        <div class="form-inline">
          <div class="input-group" data-widget="sidebar-search">
            <input class="form-control form-control-sidebar" type="search" placeholder="Search" aria-label="Search">
            <div class="input-group-append">
              <button class="btn btn-sidebar">
                <i class="fas fa-search fa-fw"></i>
              </button>
            </div>
          </div>
        </div>

        <!-- Sidebar Menu -->
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            <!-- Add icons to the links using the .nav-icon class
               with font-awesome or any other icon font library -->

            <li class="nav-item">
              <a href="${pageContext.request.contextPath}/findAllBusLine" class="nav-link">
                <i class="nav-icon fas fa-edit"></i>
                <p>
                  线路管理
                </p>
              </a>
            </li>

            <li class="nav-item">
              <a href="${pageContext.request.contextPath}/findAllStation" class="nav-link">
                <i class="nav-icon fas fa-edit"></i>
                <p>
                  站点管理
                </p>
              </a>
            </li>

            <li class="nav-item">
              <a href="${pageContext.request.contextPath}/findAllUser" class="nav-link">
                <i class="nav-icon fas fa-edit"></i>
                <p>
                  用户管理
                </p>
              </a>
            </li>

            <li class="nav-item">
              <a href="${pageContext.request.contextPath}/findAllPost2" class="nav-link">
                <i class="nav-icon fas fa-edit"></i>
                <p>
                  贴子管理
                </p>
              </a>
            </li>

            <li class="nav-item">
              <a href="${pageContext.request.contextPath}/findAllComment" class="nav-link">
                <i class="nav-icon fas fa-edit"></i>
                <p>
                  评论管理
                </p>
              </a>
            </li>

            <li class="nav-item">
              <a href="${pageContext.request.contextPath}/findAllNotice2" class="nav-link">
                <i class="nav-icon fas fa-edit"></i>
                <p>
                  通知管理
                </p>
              </a>
            </li>

            <li class="nav-item">
              <a href="${pageContext.request.contextPath}/findAllManager" class="nav-link">
                <i class="nav-icon fas fa-edit"></i>
                <p>
                  管理员管理
                </p>
              </a>
            </li>

          </ul>
        </nav>
        <!-- /.sidebar-menu -->
      </div>
      <!-- /.sidebar -->
    </aside>

    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
      <!-- Content Header (Page header) -->
      <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>线路管理</h1>
            </div>
            <div class="col-sm-6">
              <button id="addLine" type="button" class="btn btn-primary float-sm-right" data-toggle="modal"
                data-target="#modal_2">添加线路</button>
            </div>
          </div>
        </div><!-- /.container-fluid -->
      </section>

      <!-- Main content -->
      <section class="content">
        <div class="container-fluid">
          <div class="row">
            <div class="col-12">

              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">公交线路管理：增加线路，编辑线路，删除线路</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                  <table id="example1" class="table table-bordered table-striped">
                    <thead>
                      <tr>
                        <th>序号</th>
                        <th>编号</th>
                        <th>首/末班时间</th>
                        <th>票价</th>
                        <th>经过站台</th>
                        <th>操作</th>
                      </tr>
                    </thead>
                    <tbody>
                    <c:forEach items="${busLineList}" var="keyword" varStatus="id">
                      <tr>
                        <td>${id.index+1}</td>
                        <td>${keyword.lineNum}路</td>
                        <td>${keyword.beginTime}/${keyword.endTime}</td>
                        <td>${keyword.linePrice}</td>
                        <td>
                          <c:forEach items="${keyword.stations}" var="keyword2" varStatus="id2">
                            —>${keyword2.stationName}
                          </c:forEach>
                        </td>
                        <td>
                          <a onclick="asyncFind(${keyword.id})" class="btn btn-info btn-sm" href="#" data-toggle="modal" data-target="#modal_3">
                            <i class="fas fa-pencil-alt">
                            </i>
                            编辑
                          </a>
                          <a class="btn btn-danger btn-sm" href="${pageContext.request.contextPath}/deleteBusLine?id=${keyword.id}">
                            <i class="fas fa-trash">
                            </i>
                            删除
                          </a>
                        </td>
                      </tr>
                    </c:forEach>
                    </tbody>
                    <tfoot>
                      <tr>
                        <th>序号</th>
                        <th>编号</th>
                        <th>首/末班时间</th>
                        <th>票价</th>
                        <th>经过站台</th>
                        <th>操作</th>
                      </tr>
                    </tfoot>
                  </table>
                </div>
                <!-- /.card-body -->
              </div>
              <!-- /.card -->
            </div>
            <!-- /.col -->
          </div>
          <!-- /.row -->
        </div>
        <!-- /.container-fluid -->
      </section>
      <!-- /.content -->
    </div>
    <!-- /.content-wrapper -->
    <footer class="main-footer">
      <strong>Copyright &copy; 2020-2021 <a href="https://jack1024.link"> jack1024 </a>.</strong> All rights
      reserved.
    </footer>

    <!-- Control Sidebar -->
    <aside class="control-sidebar control-sidebar-dark">
      <!-- Control sidebar content goes here -->
    </aside>
    <!-- /.control-sidebar -->
  </div>
  <!-- ./wrapper -->


  <!-- Modals模态框 -->
  <section class="content">
    <div class="modal fade" id="modal_1">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">修改密码</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form action="#" method="post">
              <div class="input-group mb-3">
                <input id="password" type="password" class="form-control" placeholder="新密码">
                <div class="input-group-append">
                  <div class="input-group-text">
                    <span class="fas fa-lock"></span>
                  </div>
                </div>
              </div>
              <div class="input-group mb-3">
                <input id="rePassword" type="password" class="form-control" placeholder="重复密码">
                <div class="input-group-append">
                  <div class="input-group-text">
                    <span class="fas fa-lock"></span>
                  </div>
                </div>
              </div>
              <div class="row">
                <!-- /.col -->
                <div class="col-12">
                  <button id="modifyPassword" type="button" class="btn btn-primary btn-block">确认修改</button>
                </div>
                <!-- /.col -->
              </div>
            </form>
          </div>
        </div>
        <!-- /.modal-content -->
      </div>
      <!-- /.modal-dialog -->
    </div>
    <!-- /.modal -->

    <div class="modal fade" id="modal_2">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">添加线路</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <div class="row">
              <div class="col-lg-8">
                <div id="map1">
                  <canvas width="475" height="475" id="mycanvas1">
                    当前浏览器版本不支持，请升级浏览器
                  </canvas>
                </div>
                <div id="colorList1">
                </div>
              </div>
              <div class="col-lg-4">
                <form action="${pageContext.request.contextPath}/addLine" method="get">
                  <br>
                  <div class="form-group">
                    <input name="lineNum" type="number" class="form-control" id="exampleInputEmail1" placeholder="线路编号" required>
                  </div>
                  <div class="form-group">
                    <input name="linePrice" type="number" class="form-control" id="exampleInputEmail2" placeholder="线路票价(元)" required>
                  </div>
                  首末班时间：
                  <div class="input-group">
                    <input name="beginTime" type="time" class="form-control" placeholder="首班时间"
                           aria-label="Recipient's username" aria-describedby="button-addon2">
                    <input name="endTime" type="time" class="form-control" placeholder="末班时间"
                           aria-label="Recipient's username" aria-describedby="button-addon2">
                  </div>
                  <br>
                  <div class="input-group">
                    <input id="xPos" type="number" class="form-control" placeholder="x坐标(0—19)"
                      aria-label="Recipient's username" aria-describedby="button-addon2">
                    <input id="yPos" type="number" class="form-control" placeholder="y坐标(0—19)"
                      aria-label="Recipient's username" aria-describedby="button-addon2">
                    <div class="input-group-append">
                      <button id="addPoint" class="btn btn-outline-secondary" type="button">添加拐点</button>
                      <button id="delPoint" class="btn btn-outline-secondary" type="button">移除拐点</button>
                    </div>
                  </div>

                  <div>
                    <br>
                    <label style="font-size: 20px;">添加的拐点</label>
                    <div class="row" id="pointContainer"></div>
                  </div>
                  <br>
                  <button type="submit" class="btn btn-primary col-12">添加线路</button>
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
      <!-- /.modal-content -->
    </div>
    <!-- /.modal-dialog -->
    </div>
    <!-- /.modal -->

    <div class="modal fade" id="modal_3">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">编辑线路</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <div class="row">
              <div class="col-lg-8">
                <div id="map2">
                  <canvas width="475" height="475" id="mycanvas2">
                    当前浏览器版本不支持，请升级浏览器
                  </canvas>
                </div>
                <div id="colorList2">
                </div>
              </div>
              <div class="col-lg-4">
                <form action="${pageContext.request.contextPath}/updateLine" method="get">
                  <input id="lineId" name="lineId" type="number" class="form-control" style="display: none">
                  线路编号：
                  <div class="form-group">
                    <input id="lineNum" name="lineNum" type="number" class="form-control" readonly>
                  </div>
                  <br>
                  编辑首末班时间/票价
                  <div class="input-group">
                    <input id="beginTime" name="beginTime" type="time" class="form-control" placeholder="首班时间"
                           aria-label="Recipient's username" aria-describedby="button-addon2">
                    <input id="endTime" name="endTime" type="time" class="form-control" placeholder="末班时间"
                           aria-label="Recipient's username" aria-describedby="button-addon2">
                  </div>
                  <div class="form-group">
                    <input id="linePrice" name="linePrice" type="number" class="form-control" placeholder="线路票价(元)" required>
                  </div>
                  <button type="submit" class="btn btn-primary col-12">更新线路</button>
                  <br><br>
                  管理线路上的车站
                  <div class="input-group">
                    <input id="stationName" type="text" class="form-control" placeholder="车站名"
                           aria-label="Recipient's username" aria-describedby="button-addon2">
                    <input id="stationX" type="number" class="form-control" placeholder="x坐标(0—19)"
                           aria-label="Recipient's username" aria-describedby="button-addon2">
                    <input id="stationY" type="number" class="form-control" placeholder="y坐标(0—19)"
                           aria-label="Recipient's username" aria-describedby="button-addon2">
                    <div class="input-group-append">
                      <button id="addStation1" class="btn btn-outline-secondary" type="button">添加车站</button>
                    </div>
                  </div>
                  <div class="input-group mb-3">
                    <div class="input-group-prepend">
                      <label class="input-group-text" for="selectStation">可选车站</label>
                    </div>
                    <select class="custom-select" id="selectStation">
                      <option selected disabled="disabled">Choose...</option>
                    </select>
                    <div class="input-group-append">
                      <button id="addStation2" class="btn btn-outline-secondary" type="button">添加车站</button>
                    </div>
                  </div>
                  <hr>
                  线路上的车站：
                  <div>
                    <table class="table">
                      <thead>
                      <tr>
                        <th scope="col">#</th>
                        <th scope="col">车站名</th>
                        <th scope="col">坐标</th>
                        <th scope="col">从线路删除</th>
                      </tr>
                      </thead>
                      <tbody id="stationContainer">
                      </tbody>
                    </table>
                  </div>
                </form>
              </div>
            </div>
          </div>
        </div>
        <!-- /.modal-content -->
      </div>
      <!-- /.modal-dialog -->
    </div>
    <!-- /.modal -->

  </section>


  <!-- 弹框 -->
  <div class="alert alert-danger" role="alert">
    A simple danger alert with <a href="#" class="alert-link">an example link</a>. Give it a click if you like.
  </div>



  <!-- jQuery -->
  <script src="${pageContext.request.contextPath}/plugins/jquery/jquery.min.js"></script>
  <!-- Bootstrap 4 -->
  <script src="${pageContext.request.contextPath}/plugins/bootstrap/js/bootstrap.bundle.min.js"></script>
  <!-- SweetAlert2 -->
  <script src="${pageContext.request.contextPath}/plugins/sweetalert2/sweetalert2.min.js"></script>
  <!-- Toastr -->
  <script src="${pageContext.request.contextPath}/plugins/toastr/toastr.min.js"></script>
  <!-- DataTables  & Plugins -->
  <script src="${pageContext.request.contextPath}/plugins/datatables/jquery.dataTables.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-bs4/js/dataTables.bootstrap4.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-responsive/js/dataTables.responsive.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-responsive/js/responsive.bootstrap4.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-buttons/js/dataTables.buttons.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-buttons/js/buttons.bootstrap4.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/jszip/jszip.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/pdfmake/pdfmake.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/pdfmake/vfs_fonts.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-buttons/js/buttons.html5.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-buttons/js/buttons.print.min.js"></script>
  <script src="${pageContext.request.contextPath}/plugins/datatables-buttons/js/buttons.colVis.min.js"></script>
  <!-- AdminLTE App -->
  <script src="${pageContext.request.contextPath}/dist/js/adminlte.min.js"></script>
  <!-- AdminLTE for demo purposes -->
  <script src="${pageContext.request.contextPath}/dist/js/demo.js"></script>
  <!-- Page specific script -->
  <!-- toast弹出框 -->
  <script src="${pageContext.request.contextPath}/dist/js/popper.min.js"></script>
  <script src="${pageContext.request.contextPath}/dist/js/toast.min.js"></script>
  <script>
    $(function () {
      $("#example1").DataTable({
        "responsive": true,
        "lengthChange": false,
        "autoWidth": false,
        "buttons": ["copy", "csv", "excel", "pdf"]
      }).buttons().container().appendTo('#example1_wrapper .col-md-6:eq(0)');
      $('#example2').DataTable({
        "paging": true,
        "lengthChange": false,
        "searching": false,
        "ordering": true,
        "info": true,
        "autoWidth": false,
        "responsive": true,
      });
    });
    $(function () {
      if ('${msg}' !== "") {
        $.toast({
          title: '提示',
          subtitle: 'info',
          content: '${msg}',
          type: 'info',
          pause_on_hover: false,
          delay: 3000
        });
      }
    });
    //异步修改密码
    $("#modifyPassword").click(function (){
      $.ajax({
        data:{"password":$("#password").val(),"rePassword":$("#rePassword").val()},
        type: "POST",
        url : "${pageContext.request.contextPath}/managerModifyPasswd",
        success : function (msg){
          if(msg==="error1"){
            $.toast({
              title: '提示',
              subtitle: 'info',
              content: '两次密码输入不一致',
              type: 'info',
              pause_on_hover: false,
              delay: 3000
            });
          }
          else{
            $.toast({
              title: '提示',
              subtitle: 'info',
              content: msg,
              type: 'info',
              pause_on_hover: false,
              delay: 3000
            });
          }

        },
        error : function (msg){
          $.toast({
            title: '提示',
            subtitle: 'info',
            content: '系统错误，修改密码失败！',
            type: 'info',
            pause_on_hover: false,
            delay: 3000
          });
        }
      })
    })
  </script>

  <script>
    $('body').tooltip({
      selector: '[data-toggle="tooltip"]'
    });

    function drawMap(mapId){
      var canvas = document.getElementById(mapId);
      var ctx = canvas.getContext("2d");
      ctx.fillStyle="#FFFFFF";
      ctx.beginPath();
      ctx.fillRect(0,0,canvas.width,canvas.height);
      var num = 500 / 25;
      var col = 0;
      var row = 0;

      //绘制网格
      ctx.lineWidth = "0.2"
      for (var i = 0; i < num; i++) {
        ctx.beginPath();
        ctx.moveTo(col * 25, 0);
        ctx.lineTo(col * 25, 475);
        ctx.strokeStyle = "black";
        ctx.stroke();
        if (i % 5 == 0) {
          ctx.font = "15px Arial";
          ctx.fillText(i, col * 25, 15);
        }
        col++;
      }
      for (var i = 0; i < num; i++) {
        ctx.beginPath();
        ctx.moveTo(0, row * 25);
        ctx.lineTo(475, row * 25);
        ctx.strokeStyle = "black";
        ctx.stroke();
        if (i % 5 == 0) {
          ctx.font = "15px Arial";
          ctx.fillText(i, 5, row * 25);
        }
        row++;
      }
      //开始绘制
      $.ajax({
        data:{},
        type: "POST",
        url : "${pageContext.request.contextPath}/findMap",
        success : function (msg){
          //颜色数组
          var colorArray = ["blue","green","orange","yellow","cyan","purple","pink","brown"];
          $("#colorList1").children().remove();
          $("#colorList2").children().remove();
          for(let i=0;i<msg.busLines.length;i++){
            //绘制路径
            ctx.beginPath();
            ctx.moveTo((msg.busLines[i].points[0].x)*25, (msg.busLines[i].points[0].y)*25);
            for(let j=1;j<msg.busLines[i].points.length;j++){
              ctx.lineTo((msg.busLines[i].points[j].x)*25, (msg.busLines[i].points[j].y)*25);
            }
            ctx.lineWidth = "3"
            ctx.strokeStyle = colorArray[i];
            if(mapId==="mycanvas1"){
              $("#colorList1").append('<button type="button" style="background-color:'+colorArray[i]+'" class="btn">'+msg.busLines[i].lineNum+'号线</button>');
            }
            if(mapId==="mycanvas2"){
              $("#colorList2").append('<button type="button" style="background-color:'+colorArray[i]+'" class="btn">'+msg.busLines[i].lineNum+'号线</button>');
            }
            ctx.stroke();

            //绘制车站
            for(let j=0;j<msg.busLines[i].stations.length;j++){
              ctx.beginPath();
              ctx.arc((msg.busLines[i].stations[j].x)*25, (msg.busLines[i].stations[j].y)*25, 6, 0, 2 * Math.PI);
              ctx.fillStyle = "red";
              ctx.fill();
            }
          }
        },
        error : function (XMLHttpRequest, textStatus, errorThrown){
          alert(XMLHttpRequest.status);
          alert(XMLHttpRequest.readyState);
          alert(textStatus);
        }
      });
    }

    function deleteStationInLine(stationId){
      $.ajax({
        data:{"lineId":$("#lineId").val(),"stationId":stationId},
        type: "POST",
        url : "${pageContext.request.contextPath}/deleteStationInLine",
        success : function (msg){
          if(msg==="fail"){
            $.toast({
              title: '错误',
              subtitle: 'error',
              content: '从线路上删除车站失败！',
              type: 'error',
              pause_on_hover: false,
              delay: 2000
            });
          }
          else{
            $.toast({
              title: '成功',
              subtitle: 'success',
              content: '从线路上删除车站成功！',
              type: 'error',
              pause_on_hover: false,
              delay: 2000
            });
          }
        },
        error : function (XMLHttpRequest, textStatus, errorThrown){
          alert(XMLHttpRequest.status);
          alert(XMLHttpRequest.readyState);
          alert(textStatus);
        }
      }).then(()=>{
        asyncFind($("#lineId").val());
      })
    }

    function asyncFind(lineId) {
      $.ajax({
        data:{"lineId":lineId},
        type: "POST",
        url : "${pageContext.request.contextPath}/findBusLineById",
        success : function (msg){
          $("#lineId").val(msg.id);
          $("#lineNum").val(msg.lineNum);
          $("#beginTime").val(msg.beginTime);
          $("#endTime").val(msg.endTime);
          $("#linePrice").val(msg.linePrice);
          $("#stationContainer").children().remove();
          for(var i=0;i<msg.stations.length;i++){
            $("#stationContainer").append('<tr><th scope="row">'+(i+1)+'</th><td>'+msg.stations[i].stationName+'</td><td>('+msg.stations[i].x+','+msg.stations[i].y+')</td><td><a href="javascript:void(0);" onclick="deleteStationInLine('+msg.stations[i].id+')" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i>删除</a></td></tr>');
          }
        },
        error : function (XMLHttpRequest, textStatus, errorThrown){
          alert(XMLHttpRequest.status);
          alert(XMLHttpRequest.readyState);
          alert(textStatus);
        }
      }).then(() => {
        $.ajax({
          data:{"lineId":lineId},
          type: "POST",
          url : "${pageContext.request.contextPath}/findSelectStation",
          success : function (msg){
            $("#selectStation").children().remove();
            for(var i=0;i<msg.length;i++){
              $("#selectStation").append('<option value="'+msg[i].id+'">'+msg[i].stationName+':('+msg[i].x+','+msg[i].y+')</option>');
            }
          },
          error : function (XMLHttpRequest, textStatus, errorThrown){
            alert(XMLHttpRequest.status);
            alert(XMLHttpRequest.readyState);
            alert(textStatus);
          }
        }).then(()=>{
          drawMap("mycanvas2");
        })
      })
    }

    $("#addLine").click(function (){
      drawMap("mycanvas1");
    })

    $("#addStation1").click(function (){
      $.ajax({
        data:{"stationName":$("#stationName").val(),"x":$("#stationX").val(),"y":$("#stationY").val(),"lineId":$("#lineId").val()},
        type: "POST",
        url : "${pageContext.request.contextPath}/addStation1",
        success : function (msg){
          if(msg==="fail"){
            $.toast({
              title: '错误',
              subtitle: 'error',
              content: '车站坐标不在线路上！',
              type: 'error',
              pause_on_hover: false,
              delay: 1000
            });
          }
          else{
            $.toast({
              title: '成功',
              subtitle: 'success',
              content: '添加成功！',
              type: 'error',
              pause_on_hover: false,
              delay: 1000
            });
          }
        },
        error : function (XMLHttpRequest, textStatus, errorThrown){
          alert(XMLHttpRequest.status);
          alert(XMLHttpRequest.readyState);
          alert(textStatus);
        }
      }).then(()=>{
        asyncFind($("#lineId").val());
      })
    })

    $("#addStation2").click(function (){
      $.ajax({
        data:{"stationId":$("#selectStation").val(),"lineId":$("#lineId").val()},
        type: "POST",
        url : "${pageContext.request.contextPath}/addStation2",
        success : function (msg){
            $.toast({
                title: '成功',
                subtitle: 'success',
                content: '添加成功！',
                type: 'error',
                pause_on_hover: false,
                delay: 1000
            });
        },
        error : function (XMLHttpRequest, textStatus, errorThrown){
            alert(XMLHttpRequest.status);
            alert(XMLHttpRequest.readyState);
            alert(textStatus);
        }
      }).then(()=>{
        asyncFind($("#lineId").val());
      })
    })


    $("#delPoint").click(function (){
      $("#pointContainer").children().last().remove();
    })


    $("#addPoint").click(function () {
      var x = parseInt($("#xPos").val());
      var y = parseInt($("#yPos").val());

      if (isNaN(x) || isNaN(y)){
        $.toast({
          title: '错误',
          subtitle: 'error',
          content: '输入为空！',
          type: 'error',
          pause_on_hover: false,
          delay: 1000
        });
        return;
      }

      if (x < 0 || x > 19 || y < 0 || y > 19) {
        $.toast({
          title: '错误',
          subtitle: 'error',
          content: '坐标范围越界！',
          type: 'error',
          pause_on_hover: false,
          delay: 1000
        });
        return;
      }

      if($("#pointContainer").children().length>0) {
        var prev = $("#pointContainer").children().last().val();
        var xPrev = parseInt(prev.split(",")[0]);
        var yPrev = parseInt(prev.split(",")[1]);
        if((xPrev!==x&&yPrev!==y) || (xPrev===x&&yPrev===y)){
          $.toast({
            title: '错误',
            subtitle: 'error',
            content: '输入的坐标无法确定线路！',
            type: 'error',
            pause_on_hover: false,
            delay: 1000
          });
          return;
        }
      }

      var Pos = x + "," + y;
      var content = $('<input type="text" readonly value="' + Pos + '" class="form-control col-3" name="points">');
      $("#pointContainer").append(content);

    })
  </script>
</body>

</html>